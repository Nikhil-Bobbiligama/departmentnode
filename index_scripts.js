    ...
    {   
     ...
     // var form = $(this).parent('form');
      ...
      console.log("in view not in ajax");
        $.ajax
          ({ 
            url: 'http://192.168.10.138:3000/departments',
            type: 'GET',
            dataType: 'json',
...
//console.log(id);
              ...
              var refmodule='student';
               var depname=name;
                //$traildepname=name;
                  $.ajax({
                    url: 'http://192.168.10.138:3000/students/'+refdepid,
                    type: 'GET',
    ...
    {   var depid =deleteid;
      ...
      $.ajax({
        type : "DELETE",
        url : 'http://192.168.10.138:3000/students/'+ id,
        success : function(data) {
    ...
    {   var depid =deleteid;
      ...
      $.ajax({
        type : "DELETE",
        url : 'http://192.168.10.138:3000/departments/'+depid,
        success : function(data) {
          console.log("inside delete departments");
         loadDepartments();
        },
        error : function(data)
        {
  ...
  // body...
      // event.preventDefault();
       ...
       console.log("Inside Edit Student");
            ...
            //   $('#update-ID-A').val(editid);
             //  $('#update-name-A').val(editname);
              /// $('#update-age-A').val(editage);
               //$('#update-email-A').val(editemail);
               ///$('#update-depname-A').val(editdepname);
                  //tbodyE3.append('<h3>Edit Student Details </h3> <br>ID:<br> <input type="text" class="id" id="update-ID-A" size="25" value="'+editid+'" disabled><br>name:<br><input type="text" id="update-name-A" size="25" value="'+editname+'" ><br>Age:<br><input type ="text" id="update-age-A" size="25" value="'+editage+'"><br>Email:<br><input type ="text" id= "update-email-A" size="25" value="'+editemail+'"><br><button type="button" class="btn btn-md btn-info ups"onclick="update_student(event)" >update Student</button>');
            $(document).ready(function(){
                   $("#edit_student").modal("show");
                     });
              tbodyE3.append('<div class="modal fade" id="edit_student" role="dialog"><div class="modal-dialog modal-sm" role="document"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button><center><h2 class="modal-title" id="Modal">Edit Student</h2></center></div><div id="edit_student-body" class="modal-body"><br>ID:<br> <input type="text" class="id" id="update-ID-A" size="25" value="'+editid+'" disabled><br>Name:<br><input type="text" id="update-name-A" size="25" value="'+editname+'" ><br>Age:<br><input type ="text" id="update-age-A" size="25" value="'+editage+'"><br>Email:<br><input type ="text" id= "update-email-A" size="25" value="'+editemail+'"><input type ="hidden" id= "update-depname-A" size="25" value="'+editdepname+'" disabled><br><br><button type="button" class="btn btn-md btn-warning ups"onclick="update_student(event)" >update Student</button></div></div></div></div>');
	        //$("#edit_student").modal("show");
       ...
       // event.preventDefault();
        ...
        var tbodyE3 = $('#edit_student_div');
               ...
               //console.log(product);
                 ...
                 // tbodyE3.append('<h3>Edit Department Details </h3> <br>ID:<br> <input type="text" class="id" id="update-ID-Dep" size="25" value="'+editid+'" disabled><br>Department name:<br><input type="text" id="update-name-Dep" size="25" value="'+editname+'" ><br><br><button type="button" class="btn btn-md btn-info ups"onclick="updep(event)" >update Department</button>');
                  $(document).ready(function(){
                   $("#edit_department").modal("show");
                     });
             tbodyE3.append('<div class="modal fade" id="edit_department" role="dialog"><div class="modal-dialog modal-sm" role="document"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button><center><h2 class="modal-title" id="Modal">Edit Departmentt</h2></center></div><div id="edit_student-body" class="modal-body"><br>ID:<br> <input type="text" class="id" id="update-ID-Dep" size="25" value="'+editid+'" disabled><br>Name:<br><input type="text" id="update-name-Dep" size="25" value="'+editname+'" ><br><br><button type="button" class="btn btn-md btn-warning ups"onclick="updep(event)" >update Department</button></div></div></div></div>');         
...
//alert("add stu");
            ...
            $.ajax({
               ...
               url: 'http://192.168.10.138:3000/departments/'+depid+'/'+depname,
                type: 'PUT',
                contentType: 'json',
        ...
        function add_student(event)
          ...
          var semail= newEmail;
          var sage= newAge;
          var sdepname= newDepname;
           $.ajax({
              url: 'http://192.168.10.138:3000/students/'+sname+'/'+sage+'/'+semail+'/'+sdepname+'/'+refdepid,
           type: 'POST',
            contentType: 'json',
      ...
      function add_departments(event)
      {
              event.preventDefault();
              var refmodule='department';
        ...
        var depname =newName;
         $.ajax({
          url: 'http://192.168.10.138:3000/departments/'+depname,
           type: 'POST',
            contentType: 'json',
  ...
  // update stu
      ...
      { var refmodule='student';
        ...
        event.preventDefault();
        console.log("hellooo updatet stu...!");
        var id =$('#update-ID-A').val();
       ...
       console.log(newDepname);
        $.ajax({
           type: 'PUT',
           url : 'http://192.168.10.138:3000/students/'+id+'/'+sname+'/'+sage+'/'+semail+'/'+sdepname+'/'+sdepid,
            contentType: 'json',
            data: JSON.stringify({id: id,sname: newName,semail:newEmail,sage:newAge,sdepname:newDepname,sdepid:refdepid}),
              success: function(data) {
                console.log('sent from scripts'+ newName);
                viewstudent(this,refdepname,refdepid);
            },
